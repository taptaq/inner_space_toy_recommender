...
export function BodyPersonaQuizDialog({
  ...
  const dialogContent = (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950 px-3 py-2 sm:px-4 sm:py-2">
      <div className="relative flex h-[calc(100dvh-1rem)] w-full max-w-3xl flex-col overflow-hidden rounded-[1.5rem] border border-cyan-100/14 bg-slate-950 shadow-[0_24px_90px_rgba(8,47,73,0.34)] sm:h-[calc(100dvh-1rem)] sm:rounded-[1.75rem]">
        <div className="shrink-0 px-4 pt-4 sm:px-6 sm:pt-6">
          <div className="relative flex items-start justify-between gap-4 overflow-hidden rounded-[1.125rem] border border-cyan-100/12 bg-slate-950/88 px-4 py-4 shadow-[0_18px_38px_rgba(2,8,23,0.34)] ring-1 ring-white/[0.02] backdrop-blur-2xl sm:rounded-[1.25rem] sm:px-6 sm:py-5">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-6 sm:py-5">
          <div className="space-y-4">
            {questions.map((question, index) => (
              <section
                key={question.id}
                className="rounded-[1.125rem] border border-white/8 bg-white/[0.03] p-3.5 sm:rounded-2xl sm:p-4"
              >
        ...
        <div className="shrink-0 border-t border-cyan-100/10 px-4 py-4 sm:px-6">
