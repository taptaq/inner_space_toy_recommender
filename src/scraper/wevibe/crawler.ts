...
const TARGET_URL = 'https://wevibe.tmall.com/shop/view_shop.htm?appUid=RAzN8HWVJY3cGGRW7oQJ5Jen61ZqJNhhYB7arxnco2VeZeVeofh&spm=a21n57.1.hoverItem.1';
...
function isTargetBrandListTitle(title: string): boolean {
  return /(we[-\s]?vibe|唯倚|维依)/i.test(String(title || '').trim());
}
...
async function runCrawler() {
  console.log('--- 启动 Playwright 无头抓取引擎 [Target: We-Vibe Tmall] ---');
    ...
    while (true) {
      ...
      const pageItems = (await page.evaluate(`(() => {
        ...
        Array.from(document.querySelectorAll('.J_TItems dl.item')).forEach(function(dl, domIndex) {
          ...
          push({ domIndex: domIndex, itemId: getItemId(href), href: href, title: title, coverImage: coverImage, listDomKind: 'legacy' });
        });
        ...
        Array.from(document.querySelectorAll('.product_shelf [class*="cardContainer"], [class*="ProductShelf"] [class*="cardContainer"], [class*="product_shelf"] [class*="cardContainer"]')).forEach(function(card, domIndex) {
          ...
          push({ domIndex: domIndex, itemId: '', href: '', title: title, coverImage: coverImage, listDomKind: 'shelf' });
        });
        ...
        Array.from(document.querySelectorAll('a[href*="detail.tmall.com/item.htm"], a[href*="//detail.tmall.com/item.htm"]')).forEach(function(anchor) {
          ...
          push({ domIndex: -1, itemId: itemId, href: href, title: title, coverImage: coverImage, listDomKind: 'anchor' });
        });
      ...
      const filteredOutBrandItems = normalizedPageItems.filter((item) =>
        !isNonProductListTitle(item.title, `${item.title} ${item.listDomKind || ''}`) &&
        !isTargetBrandListTitle(item.title),
      );
      ...
      console.log(
        `  [列表统计] 原始卡片 ${normalizedPageItems.length}，伪商品过滤 ${filteredPageItems.length}，非 We-Vibe 过滤 ${filteredOutBrandItems.length}，最终保留 ${acceptedPageItems.length}`,
      );
      ...
      if (filteredOutBrandItems.length > 0) {
        console.log(`  [列表过滤] 已跳过非 We-Vibe 商品: ${filteredOutBrandItems.map((item) => item.title).join(' | ')}`);
      }
      ...
      while (retryCount <= MAX_RETRIES && !ocrText) {
        try {
          ...
          if (!detailParamsText && ocrText) {
            ...
            if (/(we[-\s]?vibe|唯倚|维依)/i.test(item.title)) {
              ocrParamPairs.push(['品牌', 'We-Vibe']);
            }
