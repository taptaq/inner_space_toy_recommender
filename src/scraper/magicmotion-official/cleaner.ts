...
export const BUFFER_PATH = path.resolve(__dirname, '../../data/magicmotion-official-review-buffer.json');
export const CLEANED_PATH = path.resolve(__dirname, '../../data/magicmotion-official-cleaned-data.json');
const RAW_TRANSLATION_CACHE_PATH = path.resolve(__dirname, '../../data/magicmotion-official-raw-description-zh-cache.json');
const BRAND_NAME = 'Magic Motion';
...
function inferAppearance(text: string): string {
  return /quiet|discreet|portable|travel|wearable|便携|静音|穿戴/i.test(text) ? 'high_disguise' : 'normal';
}
...
function inferFunctionTags(text: string): string[] {
  return uniqueStrings([
    ...
    hasAnyHint(text, ['app', 'app-controlled', 'remote', 'bluetooth']) ? '智能控制' : null,
...
export function buildNormalizedSpecs(item: CleanerBufferItem, fx: FxSnapshot): NormalizedSpecs {
  ...
  return {
    ...
    max_db: hasAnyHint(signalText, ['quiet', 'silent', 'whisper quiet']) ? 50 : null,
