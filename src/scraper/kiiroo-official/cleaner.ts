...
export const CLEANED_PATH = path.resolve(__dirname, '../../data/kiiroo-official-cleaned-data.json');
const RAW_TRANSLATION_CACHE_PATH = path.resolve(__dirname, '../../data/kiiroo-official-raw-description-zh-cache.json');
...
function inferMaterial(name: string, rawDescription: string): string {
  ...
  if (/tpe|thermoplastic elastomer/.test(text)) return 'TPE';
...
function inferAppearance(text: string): string {
  return /quiet|discreet|compact|portable|travel|hands-free|静音|便携|免提/i.test(text)
    ? 'high_disguise'
    : 'normal';
}

function inferPhysicalForm(text: string): string {
  if (/prostate|anal|insertable|internal|入体|前列腺|肛门/i.test(text)) return 'internal';
  return 'external';
}

function inferMotorType(text: string): string {
  return /powerful|intense|strong|automatic|thrusting|suction|oscillat|强劲|自动/i.test(text) ? 'strong' : 'gentle';
}
...
function inferFunctionTags(text: string, typeCode: string | null, subtypeCode: string | null): string[] {
  return uniqueStrings([
    hasAnyHint(text, ['masturbator', 'stroker', 'sleeve', 'blowjob', 'handjob']) ? '通道刺激' : null,
    hasAnyHint(text, ['app', 'app control', 'app sync']) ? 'APP控制' : null,
    hasAnyHint(text, ['interactive', 'sync', 'teledildonic', 'remote content']) ? '互动联动' : null,
    hasAnyHint(text, ['automatic', 'thrusting', 'oscillat', 'auto']) ? '自动节奏' : null,
    hasAnyHint(text, ['hands-free', 'hands free', '免提']) ? '免提' : null,
    ...
    hasAnyHint(text, ['set', 'bundle', 'kit']) ? '套装' : null,
    typeCode === 'masturbator' && subtypeCode === 'interactive_masturbator' ? '远程互动' : null,
  ]);
}
...
function collectClassifierTags(item: Record<string, unknown>, rawDescription: string): string[] {
  ...
  return uniqueStrings([
    ...rawTags,
    hasAnyHint(signalText, ['masturbator']) ? 'masturbator' : null,
    hasAnyHint(signalText, ['stroker', 'sleeve']) ? 'stroker' : null,
    hasAnyHint(signalText, ['interactive', 'sync', 'teledildonic']) ? 'interactive' : null,
    hasAnyHint(signalText, ['app', 'app control']) ? 'app' : null,
    hasAnyHint(signalText, ['automatic', 'thrusting', 'oscillat']) ? 'powered' : null,
    hasAnyHint(signalText, ['manual']) ? 'manual' : null,
    hasAnyHint(signalText, ['blowjob', 'handjob']) ? 'male_device' : null,
  ]);
}
...
function resolveMasturbatorSubtype(text: string): 'interactive_masturbator' | 'vibrating_masturbator' | 'manual_masturbator' {
  if (hasAnyHint(text, ['interactive', 'sync', 'teledildonic', 'app', 'remote content'])) {
    return 'interactive_masturbator';
  }

  if (hasAnyHint(text, ['automatic', 'thrusting', 'oscillat', 'vibration', 'powered', 'electric'])) {
...
function finalizeTypeCode(
  ...
  if (hasAnyHint(text, ['masturbator', 'stroker', 'sleeve', 'blowjob', 'handjob', 'onyx', 'keon', 'feel'])) {
    return { typeCode: 'masturbator', subtypeCode: resolveMasturbatorSubtype(text) };
  }
...
export function formatKiirooRawDescription(input: string): string {
...
async function translateForPersistence(rawDescription: string, canonicalName: string): Promise<string> {
  if (!hasMeaningfulEnglish(rawDescription)) {
    return formatKiirooRawDescription(rawDescription);
  }
  ...
  try {
    ...
    return formatKiirooRawDescription(finalizedRawDescription);
  } catch (error) {
...
export async function runCleaner(): Promise<CleanedRow[]> {
  if (!fs.existsSync(BUFFER_PATH)) {
    console.error('[中断] 未发现 Kiiroo review-buffer。');
