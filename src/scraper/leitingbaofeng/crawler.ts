...
const TARGET_URL =
  process.env.LEITINGBAOFENG_TARGET_URL ||
...
const MAX_ITEMS = Number(process.env.LEITINGBAOFENG_MAX_ITEMS || 200);
const DELAY_BETWEEN_PAGES = 3000;
const BUFFER_PATH = path.resolve(__dirname, '../../data/leitingbaofeng-review-buffer.json');
const LIST_PRICE_CACHE_PATH = path.resolve(__dirname, '../../data/leitingbaofeng-list-price-cache.json');
...
const SHELF_SCROLL_WAIT_MS = 2500;
...
async function collectListItemsFromCurrentPage(page: any, pageUrl: string, listPriceCache: ListPriceCache) {
  let lastShelfCount = 0;
  let unchangedShelfRounds = 0;
  ...
  while (true) {
    const pageItems = (await page.evaluate(`(() => {
      ...
      return {
        items: results,
        shelfCount: document.querySelectorAll('.product_shelf [class*="cardContainer"], [class*="ProductShelf"] [class*="cardContainer"], [class*="product_shelf"] [class*="cardContainer"]').length
      };
    })()`)) as {
      items: Array<{
        domIndex: number;
    ...
    const normalizedPageItems = pageItems.items.map((item) => {
    ...
    console.log(
      `  [列表商品] 当前已见 shelf=${pageItems.shelfCount}，候选=${normalizedPageItems.length}: ${normalizedPageItems.map((item, index) => `${index + 1}.${item.title}`).join(' | ')}`,
    );
    ...
    const scrollState = nextShelfScrollState({
      currentCount: pageItems.shelfCount,
      previousCount: lastShelfCount,
      unchangedRounds: unchangedShelfRounds,
      maxItems: MAX_ITEMS,
    });
    unchangedShelfRounds = scrollState.unchangedRounds;
    console.log(
      `  [懒加载] shelf ${lastShelfCount} -> ${pageItems.shelfCount} | unchanged=${unchangedShelfRounds} | action=${scrollState.reason}`,
    );

    if (!scrollState.shouldContinue || pageItems.shelfCount === 0) {
      return normalizedPageItems;
    }

    lastShelfCount = pageItems.shelfCount;
    await page.evaluate(() => window.scrollBy(0, Math.max(window.innerHeight * 1.5, 1200)));
    await page.waitForTimeout(SHELF_SCROLL_WAIT_MS);
  }
}
...
async function runCrawler() {
  console.log('--- 启动 Playwright 无头抓取引擎 [Target: 雷霆暴风 Tmall] ---');
  ...
  try {
    ...
    while (true) {
      ...
      const normalizedPageItems = await collectListItemsFromCurrentPage(page, pageUrl, listPriceCache);
      ...
      while (retryCount <= MAX_RETRIES && !ocrText) {
        try {
          ...
          if (!detailParamsText && ocrText) {
            ...
            if (
              item.title.includes('雷霆暴风') ||
              item.title.includes('LETEN') ||
              item.title.includes('leten') ||
              item.title.includes('雷霆') ||
              item.title.includes('暴风')
            ) {
              ocrParamPairs.push(['品牌', '雷霆暴风']);
            }
