...
const TARGET_URL = 'https://wangyichunfeng.tmall.com/search.htm?spm=a1z10.5-b.w5842-25615141197.2.854c33496fxx1i&search=y';
const MAX_ITEMS = Number(process.env.WANGYICHUNFENG_MAX_ITEMS || 200);
const DELAY_BETWEEN_PAGES = 3000;
const BUFFER_PATH = path.resolve(__dirname, '../../data/wangyichunfeng-review-buffer.json');
const LIST_PRICE_CACHE_PATH = path.resolve(__dirname, '../../data/wangyichunfeng-list-price-cache.json');
...
function extractLegacyListPricesFromBodyText(bodyText: string): Map<string, number> {
  const result = new Map<string, number>();
  const text = String(bodyText || '');
  if (!text) return result;

  const idBlocks = [...text.matchAll(/(?:\d{9,12}\|){2,}\d{9,12}/g)]
    .map((match) => match[0].split('|').map((item) => item.trim()).filter(Boolean))
    .filter((values) => values.length >= 3);

  const priceBlocks = [...text.matchAll(/(?:\d{1,5}(?:\.\d{1,2})?\|){2,}\d{1,5}(?:\.\d{1,2})?/g)]
    .map((match) => match[0].split('|').map((item) => item.trim()).filter(Boolean))
    .filter((values) => values.length >= 3 && values.every((value) => /^\d+(?:\.\d{1,2})?$/.test(value)));

  for (const ids of idBlocks.sort((left, right) => right.length - left.length)) {
    const prices = priceBlocks
      .filter((candidate) => candidate.length === ids.length)
      .sort((left, right) => right.length - left.length)[0];
    if (!prices) continue;

    ids.forEach((itemId, index) => {
      const numeric = Number(prices[index]);
      if (!Number.isFinite(numeric)) return;
      result.set(itemId, numeric);
    });
...
async function runCrawler() {
  console.log('--- 启动 Playwright 无头抓取引擎 [Target: 网易春风 Tmall] ---');
  ...
  try {
    ...
    while (true) {
      ...
      const hiddenPriceMap = extractLegacyListPricesFromBodyText(await page.evaluate(() => document.body?.innerText || ''));
      if (hiddenPriceMap.size > 0) {
        console.log(`  [列表价格直读] 已从旧搜索页隐藏文本解析 ${hiddenPriceMap.size} 个商品价格`);
      }
      for (const item of normalizedPageItems) {
        if (item.price !== null) continue;
        if (!item.itemId) continue;
        const hiddenPrice = hiddenPriceMap.get(item.itemId);
        if (hiddenPrice === undefined) continue;
        item.price = hiddenPrice;
        listPriceCache[buildListPriceCacheKey(item.href, item.title)] = {
          itemId: item.itemId,
          title: item.title,
          href: item.href,
          price: hiddenPrice,
      ...
      if (hiddenPriceMap.size > 0) saveListPriceCache(listPriceCache);
      ...
      while (retryCount <= MAX_RETRIES && !ocrText) {
        try {
          ...
          if (!detailParamsText && ocrText) {
            ...
            if (/(?:网易春风|春风)/.test(item.title)) {
              ocrParamPairs.push(['品牌', '网易春风']);
            }
