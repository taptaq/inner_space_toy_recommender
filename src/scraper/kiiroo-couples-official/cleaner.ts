...
export const CLEANED_PATH = path.resolve(__dirname, '../../data/kiiroo-couples-official-cleaned-data.json');
const RAW_TRANSLATION_CACHE_PATH = path.resolve(__dirname, '../../data/kiiroo-couples-official-raw-description-zh-cache.json');
...
function normalizeGenderHint(item: CleanerBufferItem, text: string): Gender {
  const hint = normalizeWhitespace(String(item.genderHint || '')).toLowerCase();
  if (hint === 'male' || hint === 'female' || hint === 'unisex') return hint;
  if (hasAnyHint(text, ['masturbator', 'stroker', 'male pleasure', 'for him', 'onyx', 'keon'])) return 'male';
  if (hasAnyHint(text, ['vibrator', 'clitoral', 'g-spot', 'rabbit', 'pearl', 'cliona'])) return 'female';
  return 'unisex';
}
...
function inferPhysicalForm(text: string): string {
  if (/masturbator|stroker|sleeve|cup|飞机杯|自慰杯/i.test(text)) return 'internal';
...
function inferMotorType(text: string): string {
  return /powerful|intense|strong|deep|rumbly|suction|thrusting|强劲|深沉|吸吮|电动/i.test(text) ? 'strong' : 'gentle';
}
...
function inferFunctionTags(text: string, typeCode: string | null, subtypeCode: string | null): string[] {
  return uniqueStrings([
    ...
    hasAnyHint(text, ['rabbit', 'dual stimulation', 'dual-stimulation']) ? '双刺激' : null,
    hasAnyHint(text, ['app', 'app control', 'remote']) ? 'APP控制' : null,
    hasAnyHint(text, ['partner', 'couples', 'shared', 'interactive']) ? '远程互动' : null,
    hasAnyHint(text, ['partner', 'couples', 'shared']) ? '伴侣共玩' : null,
    ...
    hasAnyHint(text, ['stroker', 'masturbator', 'sleeve']) ? '互动自慰杯' : null,
...
function collectClassifierTags(item: Record<string, unknown>, rawDescription: string): string[] {
  ...
  return uniqueStrings([
    ...
    hasAnyHint(signalText, ['partner', 'couples', 'shared']) ? 'couples' : null,
    hasAnyHint(signalText, ['wand']) ? 'wand' : null,
    hasAnyHint(signalText, ['stroker', 'masturbator', 'sleeve']) ? 'masturbator' : null,
    hasAnyHint(signalText, ['interactive', 'sync']) ? 'interactive' : null,
  ]);
}
...
function finalizeTypeCode(
  ...
  if (hasAnyHint(text, ['rabbit', 'dual stimulation', 'dual-stimulation'])) {
  ...
  if (hasAnyHint(text, ['masturbator', 'stroker', 'sleeve', 'interactive stroker'])) {
    return { typeCode: 'male_stroker', subtypeCode: 'powered_stroker' };
  }
...
export function buildNormalizedSpecs(item: Record<string, unknown>, fx: FxSnapshot): NormalizedSpecs {
  ...
  const gender = normalizeGenderHint(item as CleanerBufferItem, signalText);
...
export function formatKiirooCouplesRawDescription(input: string): string {
...
async function translateForPersistence(rawDescription: string, canonicalName: string): Promise<string> {
  if (!hasMeaningfulEnglish(rawDescription)) {
    return formatKiirooCouplesRawDescription(rawDescription);
  }
  ...
  try {
    ...
    return formatKiirooCouplesRawDescription(finalizedRawDescription);
  } catch (error) {
...
export async function runCleaner(): Promise<CleanedRow[]> {
  if (!fs.existsSync(BUFFER_PATH)) {
    console.error('[中断] 未发现 Kiiroo Couples review-buffer。');
