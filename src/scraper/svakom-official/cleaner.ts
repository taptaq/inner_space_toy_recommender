...
const CLEANED_PATH = path.resolve(__dirname, '../../data/svakom-official-cleaned-data.json');
...
const mapMotorType = (raw: string): string => {
  const value = (raw || '').toLowerCase();
  if (['strong', 'powerful', 'intense', '强', '高频', '大推力', '电击'].some((hint) => value.includes(hint))) return 'strong';
  return 'gentle';
};

const mapAppearance = (raw: string): string => {
  const value = (raw || '').toLowerCase();
  if (['disguise', '隐蔽', '伪装', 'discreet', 'privacy', 'wearable', '穿戴'].some((hint) => value.includes(hint)))
...
const inferDefaultMaterial = (name: string, rawDescription: string): string => {
  ...
  if (value.includes('亲肤硅胶') || value.includes('silicone') || value.includes('硅胶')) return '亲肤硅胶';
...
const resolveRmbPrice = (item: any, parsedSpecs?: any): number | null => {
  ...
  return Number.isFinite(modelRmb) && modelRmb > 0 ? Math.round(modelRmb) : null;
};

const buildDefaultSpecs = (item: any, canonicalName: string) => {
  const source = `${canonicalName}\n${item.rawDescription || ''}`;
  const explicitMaxDb = extractNoiseMaxDb(source);

  return {
    max_db: explicitMaxDb ?? 40,
    ...
    physical_form:
      /rabbit|双头|dual|g-spot|vaginal|\x61nal|\u63d2\u5165|抽插|穿戴/i.test(source)
        ? /rabbit|双头|dual/i.test(source)
          ...
          : 'internal'
        : 'external',
    motor_type: /powerful|intense|strong|高频|强震|大推力|电击/i.test(source) ? 'strong' : 'gentle',
    function_tags: dedupeTags(extractKeywordTags(source)),
...
export async function runCleaner() {
  console.log('\n======================================================');
  console.log('--- 启动 SVAKOM 官方站 AI 清洗与入库模块 ---');
  console.log('======================================================');
  ...
  if (!fs.existsSync(BUFFER_PATH)) {
    console.error('[中断] 未发现 SVAKOM 官方站 review-buffer。');
  ...
  try {
    const competitor = await withDbRetry('查询 SVAKOM 竞品', () =>
      prisma.competitors.findFirst({
        where: {
          name: { contains: 'svakom', mode: 'insensitive' },
        },
      }),
    );
    ...
    if (competitor) {
      ...
      console.log(`[关联] 已定位 SVAKOM 竞品 ID: ${brandId}`);
    } else {
      const newBrand = await withDbRetry('创建 SVAKOM 竞品', () =>
      ...
      brandId = newBrand.id;
      console.log(`[创建] 已创建 SVAKOM 竞品记录 (ID: ${brandId})`);
    }
  } catch (error) {
  ...
  for (const item of bufferData) {
    ...
    const defaultSpecs = buildDefaultSpecs(item, canonicalName);
...
3. 优先依据标题、分类、详情 OCR 文本判断商品类型与性别。
4. \`price_rmb\` 直接输出人民币数字，不要做美元换算。
...
`;
    ...
    try {
      ...
      const resolvedGender = mapGender(explicitGender || item.genderHint || parsedSpecs.gender || 'unisex');
