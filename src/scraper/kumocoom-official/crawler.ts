...
export const MAX_ITEMS = Number(process.env.KUMOCOOM_OFFICIAL_MAX_ITEMS || '250');
...
export type KumocoomProductDetail = {
  title: string;
  subtitle: string;
  metaTitle: string;
...
export type KumocoomReviewBufferItem = KumocoomListItem &
  KumocoomProductDetail & {
    isReviewed: false;
  };
...
export function normalizeProductUrl(input: string): string {
  ...
  try {
    ...
    if (!handle) return '';
    return `${url.origin}/products/${handle}`;
  } catch {
    return '';
  }
}
...
function extractListBlocks(html: string): string[] {
  const listHtml = html.match(/<[^>]+class="[^"]*product-grid[^"]*"[^>]*>([\s\S]*)/i)?.[1] || html;
  return Array.from(
    listHtml.matchAll(/<(?:li|div)\b[^>]*class="[^"]*(?:grid__item|card-wrapper|product-card-wrapper|product-item)[^"]*"[\s\S]*?<\/(?:li|div)>/gi),
  ).map((match) => match[0]);
}
...
export function extractListItemsFromHtml(html: string): KumocoomListItem[] {
  const blocks = extractListBlocks(html);
  const items: KumocoomListItem[] = [];
  ...
  for (const [index, block] of blocks.entries()) {
    ...
    const item: KumocoomListItem = {
      ...
      priceCurrency: 'USD',
      categoryHints: [],
      genderHint: inferGenderHint({ name, subtitle }),
      listPosition: index + 1,
    };

    if (sourceUrl) items.push(item);
  }
...
export function mergeUniqueListItems(items: KumocoomListItem[]): KumocoomListItem[] {
  const byUrl = new Map<string, KumocoomListItem>();
  for (const item of items) {
    ...
    byUrl.set(sourceUrl, {
      ...
      genderHint: preferred.genderHint || existing.genderHint || item.genderHint,
    });
  }
...
function extractAccordionTexts(html: string): string[] {
  const items: string[] = [];
  const gridItems = Array.from(html.matchAll(/<div\b[^>]*class="[^"]*grid__item[^"]*"[\s\S]*?<\/div>\s*<\/div>/gi));

  for (const match of gridItems) {
    const block = match[0];
    const title = normalizeWhitespace(
      block.match(/<summary[^>]*>([\s\S]*?)<\/summary>/i)?.[1] ||
        block.match(/<h[2-4][^>]*>([\s\S]*?)<\/h[2-4]>/i)?.[1] ||
        '',
    );
    const titleKey = title.toLowerCase();
    if (
      titleKey.includes('shipping') ||
      titleKey.includes('returns') ||
      titleKey.includes('refund') ||
      titleKey.includes('delivery')
    ) {
      continue;
    }
    const content = normalizeWhitespace(
      block.match(/<div\b[^>]*class="[^"]*accordion__content[^"]*"[^>]*>([\s\S]*?)<\/div>/i)?.[1] ||
        block.match(/<div\b[^>]*class="[^"]*rte[^"]*"[^>]*>([\s\S]*?)<\/div>/i)?.[1] ||
        '',
    );
    if (content) items.push([title, content].filter(Boolean).join('\n'));
  }

  return uniqueStrings(items, 60);
}

export function buildKumocoomRawDescription(parts: Array<string | null | undefined>): string {
  return uniqueStrings(parts, 220).join('\n');
}

export function extractDetailFromHtml(html: string): KumocoomProductDetail {
  const title =
  ...
  const rawDescription = buildKumocoomRawDescription([title, subtitle, metaDescription, ...accordionTexts]);
...
export function extractDetailFromShopifyProduct(product: ShopifyProduct): KumocoomProductDetail {
  const { priceSourceAmount, originalPriceSourceAmount } = resolveShopifyProductPrice(product);
  ...
  const rawDescription = buildKumocoomRawDescription([
...
function buildReviewBufferItem(item: KumocoomListItem, detail: KumocoomProductDetail): KumocoomReviewBufferItem {
  return {
    ...item,
    ...detail,
...
export function writeReviewBuffer(rows: KumocoomReviewBufferItem[]) {
  ensureDir(BUFFER_PATH);
  fs.writeFileSync(BUFFER_PATH, JSON.stringify(rows, null, 2));
}

export async function runCrawler(): Promise<KumocoomReviewBufferItem[]> {
  ...
  const rows: KumocoomReviewBufferItem[] = [];
    ...
    try {
      ...
      try {
        ...
        detail = {
          ...
          rawDescription: buildKumocoomRawDescription([htmlDetail.rawDescription, jsonDetail.rawDescription]),
        };
      } catch {}

      rows.push(buildReviewBufferItem(item, detail));
    } catch {}
  }
