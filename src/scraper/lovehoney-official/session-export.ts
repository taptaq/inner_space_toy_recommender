...
type SessionExportEnv = {
  ...
  LOVEHONEY_SESSION_CAPTURE_URL?: string;
};
...
export function resolveLovehoneySessionExportConfig(
  ...
  const configuredOutputPath = String(env.LOVEHONEY_STORAGE_STATE_PATH || '').trim() || DEFAULT_OUTPUT_RELATIVE_PATH;
  const outputPath = path.resolve(cwd, configuredOutputPath);
  const launchUrl = String(env.LOVEHONEY_SESSION_CAPTURE_URL || DEFAULT_LAUNCH_URL).trim() || DEFAULT_LAUNCH_URL;
  return {
...
async function waitForUserConfirmation() {
  ...
  try {
    await rl.question('浏览器中确认 Lovehoney 页面已正常打开后，回到这里按回车保存会话...');
  } finally {
    rl.close();
  }
}
...
export async function runLovehoneySessionExporter() {
  const config = resolveLovehoneySessionExportConfig(process.env);
  ensureDir(config.storageStatePath);

  console.log('--- 启动 Lovehoney 会话导出工具 ---');
  ...
  try {
    ...
    console.log('[会话导出] 浏览器已打开。请在浏览器中完成验证、登录或手动跳转到可正常访问的 Lovehoney 页面。');
    ...
    console.log('[会话导出] 后续可直接配合 LOVEHONEY_STORAGE_STATE_PATH 运行抓取。');
  } finally {
    await context.close();
    await browser.close();
  }
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  runLovehoneySessionExporter().catch((error) => {
    console.error('[致命错误] Lovehoney 会话导出失败:', error);
    process.exitCode = 1;
  });
}
