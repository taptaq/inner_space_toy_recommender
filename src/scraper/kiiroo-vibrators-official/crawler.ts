...
export const LIST_URL = `${ORIGIN}/collections/vibrators`;
export const COLLECTION_JSON_URL = `${ORIGIN}/collections/vibrators/products.json`;
export const BUFFER_PATH = path.resolve(__dirname, '../../data/kiiroo-vibrators-official-review-buffer.json');
export const MAX_ITEMS = Number(process.env.KIIROO_VIBRATORS_OFFICIAL_MAX_ITEMS || '200');
...
export type KiirooVibratorsListItem = {
  sourceUrl: string;
  name: string;
  subtitle: string;
  coverImage: string;
...
export type KiirooVibratorsProductDetail = ReturnType<typeof extractSharedDetailFromHtml>;

export type KiirooVibratorsReviewBufferItem = KiirooVibratorsListItem &
  KiirooVibratorsProductDetail & {
    isReviewed: false;
  };
...
export function shouldKeepKiirooVibratorsCandidate(input: Record<string, unknown>): boolean {
...
export function extractListItemsFromHtml(html: string): KiirooVibratorsListItem[] {
  ...
  const items: KiirooVibratorsListItem[] = [];
  ...
  for (const [index, block] of candidates.entries()) {
    ...
    const item: KiirooVibratorsListItem = {
      sourceUrl,
      name,
      subtitle,
      coverImage,
    ...
    if (sourceUrl && shouldKeepKiirooVibratorsCandidate(item)) {
      items.push(item);
    }
  }
...
export function extractListItemsFromShopifyJson(
  ...
  return products
    ...
    .filter((item) => item.sourceUrl && shouldKeepKiirooVibratorsCandidate(item));
}

export function mergeUniqueListItems(items: KiirooVibratorsListItem[]): KiirooVibratorsListItem[] {
  const byUrl = new Map<string, KiirooVibratorsListItem>();
...
export function buildKiirooVibratorsRawDescription(parts: Array<string | null | undefined>): string {
  return uniqueStrings(parts, 220).join('\n');
}

export function extractDetailFromHtml(html: string, accordionText = ''): KiirooVibratorsProductDetail {
  return extractSharedDetailFromHtml(html, accordionText);
}

export function extractDetailFromShopifyProduct(product: ShopifyProduct): KiirooVibratorsProductDetail {
  return extractSharedDetailFromShopifyProduct(product);
}

function normalizeDetail(detail: KiirooVibratorsProductDetail): KiirooVibratorsProductDetail {
  return {
    ...detail,
...
function buildReviewBufferItem(
  item: KiirooVibratorsListItem,
  detail: KiirooVibratorsProductDetail,
): KiirooVibratorsReviewBufferItem | null {
  if (
...
export async function crawlDetailItems(
  items: KiirooVibratorsListItem[],
  fetchDetail: (item: KiirooVibratorsListItem) => Promise<KiirooVibratorsProductDetail>,
): Promise<KiirooVibratorsReviewBufferItem[]> {
  const reviewBuffer: KiirooVibratorsReviewBufferItem[] = [];
  ...
  for (const item of items) {
    try {
      ...
      console.warn(`[kiiroo-vibrators-official] 详情抓取失败，跳过 ${item.sourceUrl}:`, error);
    }
  }
...
export async function crawlListingPages(input: CrawlListingPagesInput): Promise<KiirooVibratorsListItem[]> {
  const { fetchCollectionHtml, fetchCollectionJsonPage } = input;
  const maxItems = input.maxItems ?? MAX_ITEMS;
  const mergedCandidates: KiirooVibratorsListItem[] = [];
  ...
  try {
    ...
    console.warn('[kiiroo-vibrators-official] 列表 HTML 抓取失败，将继续尝试 Shopify JSON。', error);
  }

  const jsonItems: KiirooVibratorsListItem[] = [];
...
async function fetchVibratorDetail(
  item: KiirooVibratorsListItem,
  preloadedProduct: ShopifyProduct | null = null,
  context: BrowserContext | null = null,
): Promise<KiirooVibratorsProductDetail> {
      ...
      if (detail.rawDescription || detail.title) {
        return preloadedDetail
          ? {
              ...
              rawDescription: buildKiirooVibratorsRawDescription([detail.rawDescription, preloadedDetail.rawDescription]),
            }
          : detail;
      }
    } finally {
  ...
  try {
    ...
    console.warn(`[kiiroo-vibrators-official] 商品页 HTML 解析失败，回退 JSON: ${item.sourceUrl}`, error);
  }
...
export function writeReviewBuffer(rows: KiirooVibratorsReviewBufferItem[]) {
  ensureDir(BUFFER_PATH);
  fs.writeFileSync(BUFFER_PATH, JSON.stringify(rows, null, 2));
}

export async function runCrawler(): Promise<KiirooVibratorsReviewBufferItem[]> {
  ...
  try {
    ...
    console.warn('[kiiroo-vibrators-official] Playwright 列表抓取失败，将尝试普通 HTML。', error);
  }
  ...
  try {
    const reviewBuffer = await crawlDetailItems(listItems, async (item) => {
      ...
      return fetchVibratorDetail(item, preloadedProduct, session?.context || null);
    });
