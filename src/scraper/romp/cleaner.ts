...
const CLEANED_PATH = path.resolve(__dirname, '../../data/romp-cleaned-data.json');
...
export async function runCleaner() {
  console.log('\n======================================================');
  console.log('--- 启动 ROMP（绒谱） AI 清洗与入库模块 ---');
  console.log('======================================================');
  ...
  // --- 预搜索 ROMP 在 competitors 表中的 ID ---
  let brandId: string | null = null;
  try {
    const competitor = await prisma.competitors.findFirst({
      where: {
        OR: [
          { name: { contains: 'ROMP', mode: 'insensitive' } },
          { name: { contains: '绒谱', mode: 'insensitive' } },
        ],
      }
    });
    if (competitor) {
      ...
      console.log(`[关联] 已定位 ROMP 竞品 ID: ${brandId}`);
        ...
        brandId = newBrand.id;
        console.log(`[创建] 已创建 ROMP 竞品记录 (ID: ${brandId})`);
    }
  } catch (err) {
  ...
  for (const item of bufferData) {
    ...
    const prompt = productKind === 'care'
      ? `
你是一个个人护理耗材商品目录数据清洗助手。现有抓取至「ROMP（绒谱）」天猫店的安全套/润滑液/护理用品类商品描述：
...
`
      : productKind === 'pad'
      ? `
你是一个家居床品防护垫商品目录数据清洗助手。现有抓取至「ROMP（绒谱）」天猫店的床事垫/防水垫/护理垫类商品描述：
...
`
      : productKind === 'apparel'
      ? `
你是一个服装商品目录数据清洗助手。现有抓取至「ROMP（绒谱）」天猫店的服饰类商品描述：
...
`
      : `
你是一个专注处理个人护理器具参数的数据拆解机器人。现有抓取至「ROMP（绒谱）」天猫店的纯文本描述：
...
`;
    ...
    try {
      ...
      const existingProduct = await prisma.products.findFirst({
        where: brandId ? { name: canonicalName, competitor_id: brandId } : { name: canonicalName },
      });
      ...
      await prisma.recommender_items.deleteMany({ where: { name: canonicalName, brand: '绒谱' } });
