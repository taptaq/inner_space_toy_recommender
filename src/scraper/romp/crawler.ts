...
import { isRompBrandLikeText } from './brand-rules';
...
const TARGET_URL = 'https://rongpucryp.tmall.com/shop/view_shop.htm?appUid=RAzN8HWSmfzT5RT1SLHXyLL6kMetQtqtXNZMEEU5m23mNxu1cQs&spm=a21n57.1.hoverItem.3';
const MAX_ITEMS = Number(process.env.ROMP_MAX_ITEMS || 200);
const DELAY_BETWEEN_PAGES = 3000;
const BUFFER_PATH = path.resolve(__dirname, '../../data/romp-review-buffer.json');
const LIST_PRICE_CACHE_PATH = path.resolve(__dirname, '../../data/romp-list-price-cache.json');
...
async function runCrawler() {
  console.log('--- 启动 Playwright 无头抓取引擎 [Target: ROMP Tmall] ---');
  ...
  try {
    ...
    while (true) {
      ...
      const filteredPromoItems = normalizedPageItems.filter((item) =>
        isNonProductListTitle(item.title, `${item.title} ${item.listDomKind || ''}`),
      );
      const promoAcceptedPageItems = normalizedPageItems.filter((item) =>
        !isNonProductListTitle(item.title, `${item.title} ${item.listDomKind || ''}`),
      );
      const filteredBrandItems = promoAcceptedPageItems.filter(
        (item) => !isRompBrandLikeText(`${item.title} ${item.listDomKind || ''}`),
      );
      const acceptedPageItems = promoAcceptedPageItems.filter((item) =>
        isRompBrandLikeText(`${item.title} ${item.listDomKind || ''}`),
      );
      if (filteredPromoItems.length > 0) {
        console.log(`  [列表过滤] 已跳过非商品卡片: ${filteredPromoItems.map((item) => item.title).join(' | ')}`);
      }
      if (filteredBrandItems.length > 0) {
        console.log(`  [列表过滤] 已跳过非 ROMP/绒谱 商品: ${filteredBrandItems.map((item) => item.title).join(' | ')}`);
      }
      console.log(
        `  [列表统计] 原始卡片=${normalizedPageItems.length} | 伪商品过滤=${filteredPromoItems.length} | 品牌过滤=${filteredBrandItems.length} | 最终保留=${acceptedPageItems.length}`,
      );
      ...
      while (retryCount <= MAX_RETRIES && !ocrText) {
        try {
          ...
          if (!detailParamsText && ocrText) {
            ...
            if (isRompBrandLikeText(item.title)) {
              ocrParamPairs.push(['品牌', 'ROMP']);
            }
