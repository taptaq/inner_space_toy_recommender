...
const TARGET_URL =
  process.env.POPOCAT_TARGET_URL ||
...
const MAX_ITEMS = Number(process.env.POPOCAT_MAX_ITEMS || 200);
const DELAY_BETWEEN_PAGES = 3000;
const BUFFER_PATH = path.resolve(__dirname, '../../data/popocat-review-buffer.json');
const LIST_PRICE_CACHE_PATH = path.resolve(__dirname, '../../data/popocat-list-price-cache.json');
const LEGACY_LIST_AREA_SELECTOR = '.J_TItems';
const LEGACY_LIST_CARD_SELECTOR = '.J_TItems dl.item';
const SHELF_LIST_AREA_SELECTOR =
  '[data-spm="product_shelf"], .product_shelf, [class*="ProductShelf"], [class*="product_shelf"]';
const SHELF_LIST_CARD_SELECTOR =
  '[data-spm="product_shelf"] [class*="cardContainer"], .product_shelf [class*="cardContainer"], [class*="ProductShelf"] [class*="cardContainer"], [class*="product_shelf"] [class*="cardContainer"]';
...
async function runCrawler() {
  console.log('--- 启动 Playwright 无头抓取引擎 [Target: POPOCAT Tmall] ---');
  ...
  try {
    ...
    while (true) {
      ...
      const pageItems = (await page.evaluate(`(() => {
        ...
        Array.from(document.querySelectorAll('[data-spm="product_shelf"] [class*="cardContainer"], .product_shelf [class*="cardContainer"], [class*="ProductShelf"] [class*="cardContainer"], [class*="product_shelf"] [class*="cardContainer"]')).forEach(function(card, domIndex) {
