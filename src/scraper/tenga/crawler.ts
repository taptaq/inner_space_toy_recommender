...
const TARGET_URL = 'https://tenga.tmall.com/category.htm?spm=a1z10.1-b-s.w5001-25428877010.3.8dda25318B3Mvr&search=y&scene=taobao_shop';
const MAX_ITEMS = Number(process.env.TENGA_MAX_ITEMS || 200);
const DELAY_BETWEEN_PAGES = 3000;
const BUFFER_PATH = path.resolve(__dirname, '../../data/tenga-review-buffer.json');
const LIST_PRICE_CACHE_PATH = path.resolve(__dirname, '../../data/tenga-list-price-cache.json');
...
async function runCrawler() {
  console.log('--- 启动 Playwright 无头抓取引擎 [Target: TENGA Tmall] ---');
      ...
      while (retryCount <= MAX_RETRIES && !ocrText) {
        try {
          ...
          if (!detailParamsText && ocrText) {
            ...
            if (/tenga/i.test(item.title)) {
              ocrParamPairs.push(['品牌', 'TENGA']);
            }
